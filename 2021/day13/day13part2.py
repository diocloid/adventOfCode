...
def fold(paper, size, passedfolddata):
    ...
    if(foldinfo[0] == 'y'):
        paperlength = int(foldinfo[1])
        paperwidth =  len(paper[0])
        ...
        for i in range(int(foldinfo[1])-1, -1, -1):
            ...
            for y in range(0, paperwidth, +1):
                if(paper[len(paper)-i-1][y] == '#'):
                    foldedpaper[i][y] = paper[len(paper)-i-1][y]
