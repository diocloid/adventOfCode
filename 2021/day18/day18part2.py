...
def main(args):
    ...
    for x in range(len(parsedinput)):
        for y in range(len(parsedinput)):
            if (x != y):
                testinput = []
                firsttestyarray = copy.deepcopy(parsedinput[x])
                secondtestarray = copy.deepcopy(parsedinput[y])
                testinput.append(firsttestyarray)
                testinput.append(secondtestarray)
                #print (testinput)
                result = calculate(testinput)
                magnitude = checkmagnitude(result)
                if(magnitude > largestsum):
                    largestsum = magnitude
    
    print(f'result {largestsum}')
